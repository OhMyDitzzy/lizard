...
void reportError(const LizardError& err, const std::string& source) {
    ...
    std::cerr
        << "\n"
        << ansi::BOLD_RED << "error[" << err.code << "]: " << ansi::RESET
        << ansi::BOLD     << err.message                   << ansi::RESET << "\n"

        // " --> filepath:line:col"  — indented by gutter width
        << std::string(gw, ' ')
        << ansi::BLUE << " --> " << ansi::RESET
        << err.filepath << ":" << err.line << ":" << err.col << "\n"
        ...
        << std::string(gw + 1, ' ')
        << ansi::BLUE << "| " << ansi::RESET
        << ansi::BOLD_RED << underline << ansi::RESET << "\n"
